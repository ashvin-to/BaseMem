...
SKILLS: Named workflows are available as skills. Before starting a multi-step task, check if a skill exists: code-review, session-start, explore-codebase, debug-issue, task-workflow. Using a skill reduces tool calls and token usage. Skills are installed under skills/using-basemem/ (or agent equivalent). Load a skill by reading its SKILL.md file (e.g. skills/using-basemem/code-review/SKILL.md) and following the workflow table.
