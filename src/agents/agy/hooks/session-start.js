...
const PLUGIN_HOME = process.env.AGY_PLUGIN_ROOT
  ? path.resolve(process.env.AGY_PLUGIN_ROOT)
  : path.join(os.homedir(), '.gemini', 'antigravity-cli', 'plugins', 'basemem');

const flagFile = path.join(PLUGIN_HOME, '.basemem-active');
const resolvedHome = fs.realpathSync(PLUGIN_HOME);
