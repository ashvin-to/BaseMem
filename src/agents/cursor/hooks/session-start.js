...
const rulesFile = path.join(configDir, 'rules', 'basemem.mdc');
const integrity = checkRulesIntegrity('cursor', rulesFile);
if (!integrity.intact) {
  process.stderr.write('BaseMem rules missing from cursor config — run: node bin/lib/install.js repair\n');
}
...
emitHookOutput('cursor', contextResult, rulesText);
